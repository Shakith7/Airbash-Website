...
const PricingSection = () => {
  const plans = [
    {
      title: "Basic",
      description: "For small functions",
      price: "$99",
      period: "/monthly",
      features: [
    ...
    },
    {
      ...
      buttonText: "Get started",
      highlight: true, // This is the highlighted plan
    },
    {
      title: "Enterprise",
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {plans.map((plan, index) => (
          ...
          >
            {/* Badge for Highlight */}
            {plan.highlight && (
              <div className="absolute top-0 right-0 bg-primaryCricket text-white text-xs font-bold px-2 py-1 rounded-bl-lg">
                Popular
              </div>
            )}
            ...
            <ul className="text-gray-600 mb-6">
              {plan.features.map((feature, i) => (
                <li key={i} className="flex items-center space-x-2 mb-2">
                  <span className="text-primaryCricket">✔</span>
                  <span>{feature}</span>
                </li>
              ))}
            </ul>
